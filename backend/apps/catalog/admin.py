...
class CategoryAdmin(ImportExportModelAdmin):
    ...
    product_count.short_description = "Item Count"
...
class ProductAdmin(ImportExportModelAdmin):
    ...
    def stock_display(self, obj):
        if obj.stock == 0:
            color, label = '#e63329', 'Out of Stock'
        elif obj.stock <= 5:
            color, label = '#f59e0b', f'{obj.stock} Left'
        else:
            color, label = '#10b981', f'{obj.stock} Left'
        return format_html('<span style="color:{};font-weight:bold">{}</span>', color, label)
    stock_display.short_description = "Stock"

    def discount_percent_display(self, obj):
        return f"{obj.discount_percent}%"
    discount_percent_display.short_description = "Discount Percentage"
